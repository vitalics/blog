---
title: Using dotenv-guards library for safe environment variables
description: At this article I'll share about dotenv-guards library and how it solves our issues in current project
date: 2023-05-31
image: '/blog/03-dotenv/03-dotenv.webp'
tags: ['dotenv', 'typescript', 'ts', 'library']
authors: ['vitalics']
...
```ts title="src/utils/env/index.ts" showLineNumbers
...
```ts title="src/utils/env/register.ts" showLineNumbers
...
```ts title="src/utils/env/guards/number.ts" showLineNumbers
type Options = {
...
```ts title="utils/env/register.ts" showLineNumbers
...
```ts title="example.ts" caption="usage file" showLineNumbers
...
```tsx title="src/utils/env.ts" showLineNumbers caption="example from https://github.com/typestack/class-validator#usage"
import {
  validate,
  validateOrReject,
  IsInt,
