...
date: 2023-05-31
image: '/blog/04-jsx-telegram/04-jsx-telegram.webp'
tags: ['react', 'preact', 'telegram']
authors: ['vitalics']
...
```tsx title="html/bold.tsx" showLineNumbers
...
```tsx title="html/render.tsx" showLineNumbers
...
```tsx title="markdown/bold.tsx" showLineNumbers
...
```tsx title="markdown/render.tsx" showLineNumbers
...
```tsx
export default function render(node: VNode | JSX.Element) {
  const text = toString(node) // html-like string
...
```tsx title="example.ts" caption="Example os usage"
...
![Image1. Result of the executed command](/blog/04-jsx-telegram/04-result.png)
