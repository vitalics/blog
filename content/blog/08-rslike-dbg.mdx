...
date: 2023-09-05
image: '/blog/08-rslike-dbg/08-console-log-hero.webp'
...
```ts title="some.ts" showLineNumbers camption="the problem"
const a = 123
...
```ts title="some.ts" showLineNumbers
...
```ts title="advanced/delimiter.ts" showLineNumbers
import { dbg } from '@rslike/dbg'
...
```ts title="advanced/output.ts" showLineNumbers
import { dbg } from '@rslike/dbg'
...
```ts title="advanced/result.ts" showLineNumbers
import { dbg } from '@rslike/dbg'
...
```ts title="anatomy.ts" caption="example of dbg output"
const a = 123;
