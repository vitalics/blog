...
const makeSVGIcon = (svgString: string) => {
  ...
  const svg = root.children[0] as any;
  svg.properties = {
    ...svg.properties,
    width: 16,
    height: 16,
  ...
  return (tree, file) => {
    ...
    visit(tree, 'element', (node: any) => {
      ...
      node.children = node.children.filter(
        (child: any) => child.type === 'comment' || child.type !== 'text' || !/^\n+$/.test(child.value)
      );
      ...
      const isDirectory =
        ...
        otherChildren.some((child: any) => child.type === 'element' && child.tagName === 'ul');
