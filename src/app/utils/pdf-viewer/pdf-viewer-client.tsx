...
import {
  FileText, Upload, Download, Share2, ArrowLeft,
  ChevronLeft, ChevronRight, ZoomIn, ZoomOut, X,
  Type, Trash2, MousePointer, ScanLine, QrCode, Barcode, FilePlus, Plus, Minus,
  Save, Undo2, Redo2, Clock, FolderOpen, CaseSensitive, FileSearch, Copy, Check,
  Shapes, Square, Circle, Pencil, PencilLine,
  PenLine, RotateCcw, ChevronsUpDown, Menu, SlidersHorizontal,
} from 'lucide-react'
            ...
            <CommandGroup>
              {OCR_LANGUAGES.map((l) => (
                <CommandItem
                  key={l.value}
                  value={l.label}
                  onSelect={() => { onChange(l.value); setOpen(false) }}
                  className="text-sm"
                >
                  <Check className={['mr-2 h-3.5 w-3.5', value === l.value ? 'opacity-100' : 'opacity-0'].join(' ')} />
                  {l.label}
                </CommandItem>
              ))}
            </CommandGroup>
...
type EditorMode = 'select' | 'add-text' | 'area-scan' | 'area-ocr' | 'add-qr' | 'add-barcode' | 'add-shape' | 'draw' | 'add-signature'
...
type Annotation = TextAnnotation | ImageAnnotation | ShapeAnnotation | DrawAnnotation | SignatureAnnotation
...
export default function PdfViewerPage() {
  ...
  const [scanStatus, setScanStatus] = useState<'idle' | 'scanning' | 'found' | 'notfound' | 'ocr-found'>('idle')

  // Shape tool state
  const [shapeType, setShapeType] = useState<'circle' | 'rect' | 'line'>('rect')
  ...
  const dragRef = useRef<{ id: string; startX: number; startY: number; origX: number; origY: number; origPoints?: { x: number; y: number }[]; origX2?: number; origY2?: number } | null>(null)
  const resizeRef = useRef<{ id: string; startX: number; startY: number; origW: number; origH: number } | null>(null)
  const moveRafRef = useRef<number | null>(null)
  const lineEndpointRef = useRef<{ id: string; end: 'start' | 'end' } | null>(null)
  ...
  useEffect(() => { pdfFileRef.current = pdfFile }, [pdfFile])
  ...
  useEffect(() => {
    idbGetAll()
      .then((all) => setSavedSessions(all.sort((a, b) => b.savedAt - a.savedAt)))
      .catch(() => {})
    sigGetAll()
      .then((all) => setSavedSignatures(all.sort((a, b) => b.createdAt - a.createdAt)))
      .catch(() => {})
  }, [])
  ...
  const pushHistory = useCallback((next: Annotation[], before?: Annotation[]) => {
    undoStack.current = [...undoStack.current, before ?? annotationsRef.current]
    redoStack.current = []
    setAnnotations(next)
    setCanUndo(true)
    setCanRedo(false)
  ...
  const loadPdf = useCallback(async (
    ...
    try {
      ...
      setCanShare(
        ...
        navigator.canShare({ files: [new File([arrayBuffer], file.name, { type: 'application/pdf' })] })
      )
    } catch (err) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (err instanceof Error && err.name !== 'RenderingCancelledException') setErrorMsg(err.message)
      }
    })()
  ...
  const handleFile = useCallback((file: File) => { setPdfFile(file); loadPdf(file) }, [loadPdf])
  ...
  const handleClear = () => {
    ...
    setTotalPages(0); setScale(1.0); setCanShare(false); setAnnotations([])
    setSelectedId(null); setEditingId(null); setMode('select')
    pdfDocRef.current = null; pdfBytesRef.current = null
  ...
  const saveSignature = async () => {
    ...
    const sig: StoredSignature = { id: uid(), name: sigName.trim() || 'Signature', dataUrl, createdAt: Date.now() }
  ...
  const placeSignature = (sig: StoredSignature) => {
    const annotation: SignatureAnnotation = {
      id: uid(), page: currentPage,
      xRatio: 0.1, yRatio: 0.1, wRatio: 0.3, hRatio: 0.15,
      dataUrl: sig.dataUrl, sigId: sig.id,
    }
  ...
  const handleLineEndpointDown = (e: React.PointerEvent<SVGCircleElement>, id: string, end: 'start' | 'end') => {
    e.stopPropagation()
    e.preventDefault()
  ...
  const handleOverlayPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (mode === 'add-text') {
      ...
      const annotation: TextAnnotation = {
        id: uid(), page: currentPage, ...ratio,
        text: 'Text', fontSize: newFontSize, color: newColor, bgColor: newBgColor,
      }
      pushHistory([...annotationsRef.current, annotation])
      setSelectedId(annotation.id)
  ...
  const handleOverlayPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if ((mode === 'area-scan' || mode === 'area-ocr') && selectStartRef.current) {
      const canvas = canvasRef.current
      if (!canvas) return
      const rect = canvas.getBoundingClientRect()
      const x = e.clientX - rect.left
      const y = e.clientY - rect.top
      setSelectRect({ x1: selectStartRef.current.x, y1: selectStartRef.current.y, x2: x, y2: y })
      return
    }
    if (mode === 'add-shape' && shapeStartRef.current) {
      ...
      if (!canvas) return
      const rect = canvas.getBoundingClientRect()
      setShapeRect({ x1: shapeStartRef.current.x, y1: shapeStartRef.current.y, x2: e.clientX - rect.left, y2: e.clientY - rect.top })
      return
    }
  ...
  const handleOverlayPointerUp = async (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (mode === 'add-shape' && shapeStartRef.current && shapeRect) {
      ...
      if (canvas) {
        const rect = canvas.getBoundingClientRect()
        if (shapeType === 'line') {
          const dist = Math.hypot(shapeRect.x2 - shapeRect.x1, shapeRect.y2 - shapeRect.y1)
          if (dist > 5) {
            const annotation: ShapeAnnotation = {
              id: uid(), page: currentPage,
              xRatio: shapeRect.x1 / rect.width, yRatio: shapeRect.y1 / rect.height,
              x2Ratio: shapeRect.x2 / rect.width, y2Ratio: shapeRect.y2 / rect.height,
              wRatio: 0, hRatio: 0,
              shape: 'line',
          ...
          if (x2 - x1 > 5 && y2 - y1 > 5) {
            const annotation: ShapeAnnotation = {
              id: uid(), page: currentPage,
              xRatio: x1 / rect.width, yRatio: y1 / rect.height,
              wRatio: (x2 - x1) / rect.width, hRatio: (y2 - y1) / rect.height,
              shape: shapeType,
              strokeColor: newStrokeColor,
    ...
    if ((mode !== 'area-scan' && mode !== 'area-ocr') || !selectStartRef.current || !selectRect) return
    ...
    if (!ctx) return
    ctx.drawImage(canvas, x1 * scaleX, y1 * scaleY, w * scaleX, h * scaleY, 0, 0, offscreen.width, offscreen.height)
    ...
    const placeText = (text: string) => {
      const annotation: TextAnnotation = {
        id: uid(), page: currentPage, xRatio, yRatio,
        text, fontSize: newFontSize, color: newColor, bgColor: newBgColor,
      }
      pushHistory([...annotationsRef.current, annotation])
      setSelectedId(annotation.id)
    }
    ...
    try {
      ...
      if ('BarcodeDetector' in window) {
        ...
        const detector = new (window as any).BarcodeDetector({ formats: ['qr_code', 'code_128', 'code_39', 'ean_13', 'ean_8', 'upc_a', 'upc_e', 'itf', 'codabar', 'data_matrix', 'pdf417', 'aztec'] })
  ...
  const handleAnnotationPointerDown = (e: React.PointerEvent, id: string) => {
    ...
    dragRef.current = {
      id, startX: e.clientX, startY: e.clientY, origX, origY,
  ...
  const handleAnnotationPointerMove = (e: React.PointerEvent) => {
    ...
    moveRafRef.current = requestAnimationFrame(() => {
      ...
      setAnnotations((prev) => prev.map((a) => {
        ...
        if (isShape(a) && a.shape === 'line' && drag.origX2 !== undefined && drag.origY2 !== undefined) {
          return {
            ...a,
        ...
        return { ...a, xRatio: Math.max(0, Math.min(1, drag.origX + dx)), yRatio: Math.max(0, Math.min(1, drag.origY + dy)) }
      }))
    })
  }
  ...
  const handleResizePointerDown = (e: React.PointerEvent, id: string) => {
    ...
    resizeRef.current = { id, startX: e.clientX, startY: e.clientY, origW: ann.wRatio, origH: ann.hRatio }
  ...
  const placeQrCode = async () => {
    ...
    try {
      ...
      const offscreen = document.createElement('canvas')
      await QRCode.toCanvas(offscreen, genText.trim(), { width: 200, errorCorrectionLevel: 'M' })
      const dataUrl = offscreen.toDataURL('image/png')

      const annotation: ImageAnnotation = {
        id: uid(), page: currentPage,
        xRatio: 0.1, yRatio: 0.1,
        wRatio: 0.25, hRatio: 0.25,
        dataUrl, label: `QR: ${genText.slice(0, 30)}`,
      }
      pushHistory([...annotationsRef.current, annotation])
      setSelectedId(annotation.id)
      setMode('select')
  ...
  const placeBarcodeImage = async () => {
    ...
    try {
      ...
      const wRatio = 0.4
      const hRatio = wRatio * aspectRatio * ((canvas?.width ?? 600) / (canvas?.height ?? 800))

      const annotation: ImageAnnotation = {
        id: uid(), page: currentPage,
        xRatio: 0.1, yRatio: 0.1,
        wRatio, hRatio,
        dataUrl, label: `${genBarcodeFormat}: ${genText.slice(0, 20)}`,
      }
      pushHistory([...annotationsRef.current, annotation])
      setSelectedId(annotation.id)
      setMode('select')
      ...
      setErrorMsg(err instanceof Error ? err.message : 'Barcode generation failed. Is jsbarcode installed?')
    }
  }
  ...
  const buildAnnotatedPdfBytes = async (): Promise<Uint8Array> => {
    ...
    function drawPdfArrow(page: any, from: {x:number,y:number}, to: {x:number,y:number}, color: any, size: number) {
      ...
      page.drawLine({ start: to,   end: left,  color, thickness, lineCap: LineCapStyle.Round })
      page.drawLine({ start: to,   end: right, color, thickness, lineCap: LineCapStyle.Round })
    }
    ...
    for (const ann of annotationsRef.current) {
      ...
      if (isSignature(ann)) {
        ...
        pdfPage.drawImage(img, { x, y, width: ann.wRatio * pw, height: ann.hRatio * ph })
      } else if (isImage(ann)) {
        const res = await fetch(ann.dataUrl)
        const bytes = await res.arrayBuffer()
        const img = await pdfDoc.embedPng(bytes)
        ...
        if (ann.shape === 'line') {
          ...
          pdfPage.drawLine({
            ...
            color: strokeCol, thickness: ann.strokeWidth, lineCap: LineCapStyle.Round,
          })
          const arrowSize = Math.max(ann.strokeWidth * 4, 8)
          if (ann.arrowEnd)   drawPdfArrow(pdfPage, { x: x1, y: y1 }, { x: x2, y: y2 }, strokeCol, arrowSize)
          if (ann.arrowStart) drawPdfArrow(pdfPage, { x: x2, y: y2 }, { x: x1, y: y1 }, strokeCol, arrowSize)
        } else {
          const x = ann.xRatio * pw
          ...
          const fillColor = ann.fillColor !== 'none' ? (() => { const { r, g, b } = parseHexToRgb01(ann.fillColor); return rgb(r, g, b) })() : undefined
          if (ann.shape === 'circle') {
            pdfPage.drawEllipse({
              x: x + w / 2, y: y + h / 2, xScale: w / 2, yScale: h / 2,
              borderColor: strokeCol, borderWidth: ann.strokeWidth,
              color: fillColor, opacity: fillColor ? 1 : 0, borderOpacity: 1,
            })
          } else {
        ...
        if (ann.bgColor !== 'none') {
          ...
          pdfPage.drawRectangle({ x, y, width: tw, height: ann.fontSize, color: rgb(br, bg, bb) })
        }
        pdfPage.drawText(ann.text, { x, y, size: ann.fontSize, font, color: rgb(r, g, b) })
      }
    }
  ...
  const handleOcrCopy = () => {
    navigator.clipboard.writeText(pageOcrText).then(() => {
      setOcrCopied(true)
  ...
  const handleShare = () => {
    ...
    setIsSharing(true)
    navigator.share({ files: [shareFileRef.current] }).catch((err) => {
  ...
  const updateSelectedShape = (patch: Partial<ShapeAnnotation & DrawAnnotation>) => {
    if (!selectedId) return
    setAnnotations((prev) => prev.map((a) =>
      a.id === selectedId && (isShape(a) || isDraw(a)) ? { ...a, ...patch } as Annotation : a
    ))
  }

  const commitSelectedShapeUpdate = (patch: Partial<ShapeAnnotation & DrawAnnotation>) => {
    if (!selectedId) return
  ...
  // Overlay cursor
  const overlayCursor =
    mode === 'add-text' ? 'cursor-crosshair' :
    mode === 'area-scan' ? 'cursor-crosshair' :
    mode === 'area-ocr' ? 'cursor-crosshair' :
    mode === 'add-shape' ? 'cursor-crosshair' :
  ...
  return (
    ...
    <div className="mx-auto max-w-7xl px-4 py-12">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">View and annotate PDF files in your browser. Nothing is uploaded.</p>
      </div>
      ...
      {savedSessions.length > 0 && status === 'idle' && (
        <div className="mb-6">
          ...
          <ul className="space-y-2">
            {savedSessions.map((s) => (
              <li key={s.id} className="flex items-center gap-3 rounded-lg border bg-card p-3 transition-colors hover:bg-muted/50">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatBytes(s.fileSize)} · {s.totalPages} {s.totalPages === 1 ? 'page' : 'pages'} · {s.annotationCount} annotation{s.annotationCount !== 1 ? 's' : ''} · {new Date(s.savedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
                <Button size="sm" variant="outline" className="shrink-0 gap-1.5" onClick={() => handleOpenSession(s)}>
                  <FolderOpen className="h-3.5 w-3.5" /> Open
                </Button>
                <Button size="icon" variant="ghost" className="h-8 w-8 shrink-0 text-muted-foreground hover:text-destructive" aria-label="Delete session" onClick={() => handleDeleteSession(s.id)}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex min-h-56 flex-col items-center justify-center gap-4 rounded-xl border-2 border-dashed border-muted-foreground/25 p-8 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Start from scratch (A4)</p>
            </div>
            <div className="flex w-full max-w-xs items-center gap-2">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') createNewPdf() }}
            ...
            <Button size="sm" className="gap-1.5" onClick={(e) => { e.stopPropagation(); createNewPdf() }}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-lg border bg-card p-2">
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              <span className="shrink-0 text-xs text-muted-foreground">{formatBytes(pdfFile.size)}</span>
              <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={handleClear} aria-label="Close PDF">
                <X className="h-3.5 w-3.5" />
              </Button>
            </div>
            <div className="flex items-center gap-1">
              <Button variant="outline" size="icon" onClick={goToPrev} disabled={currentPage <= 1} aria-label="Previous page">
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <span className="min-w-[5rem] text-center text-sm tabular-nums">{currentPage} / {totalPages}</span>
              <Button variant="outline" size="icon" onClick={goToNext} disabled={currentPage >= totalPages} aria-label="Next page">
                <ChevronRight className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={addPage} aria-label="Add page after current">
                <Plus className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={deletePage} aria-label="Delete current page">
                <Minus className="h-4 w-4" />
              </Button>
            </div>
            <div className="flex items-center gap-1">
              <Button variant="outline" size="icon" onClick={zoomOut} disabled={scale <= SCALE_MIN} aria-label="Zoom out">
                <ZoomOut className="h-4 w-4" />
              </Button>
              <span className="min-w-[3.5rem] text-center text-sm tabular-nums">{Math.round(scale * 100)}%</span>
              <Button variant="outline" size="icon" onClick={zoomIn} disabled={scale >= SCALE_MAX} aria-label="Zoom in">
            ...
            <div className="flex items-center gap-1">
              ...
              <Button size="sm" className="gap-1.5" onClick={handleExport} disabled={isExporting}>
                <Download className="h-3.5 w-3.5" />
                {isExporting ? 'Exporting…' : 'Export PDF'}
              </Button>
              <span className={`flex items-center gap-1 text-xs text-green-600 dark:text-green-400${!sessionSaved ? ' invisible' : ''}`}>
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="outline" size="icon" className="lg:hidden" aria-label="Properties">
                  <SlidersHorizontal className="h-4 w-4" />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="flex w-72 flex-col gap-0 overflow-y-auto p-0">
                <SheetTitle className="border-b px-4 py-3 text-sm font-semibold">Properties</SheetTitle>

                {/* OCR language */}
                {(mode === 'area-ocr' || mode === 'area-scan') && (
                  <div className="border-b p-3">
                    <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">OCR language</p>
                ...
                <div className="border-b p-3">
                  <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    {selectedIsShape ? 'Shape style' : selectedIsDraw ? 'Stroke style' : 'Text style'}
                  </p>
                  ...
                  {(selectedIsShape || mode === 'add-shape') && (
                    <>
                      <div className="mb-2 flex items-center justify-between gap-2">
                        ...
                        <ColorPicker value={newStrokeColor} onChange={setNewStrokeColor} onBlur={(hex) => commitSelectedShapeUpdate({ strokeColor: hex })} />
                      </div>
                      {shapeType !== 'line' && !(selectedIsShape && (selectedAnn as ShapeAnnotation)?.shape === 'line') && (
                        <div className="mb-2 flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1">
                            <ColorPicker
                              ...
                              onBlur={(hex) => commitSelectedShapeUpdate({ fillColor: hex })}
                            />
                            <button type="button"
                              title={newFillColor === 'none' ? 'Enable fill' : 'Remove fill'}
                              onClick={() => { const next = newFillColor === 'none' ? '#ffffff' : 'none'; setNewFillColor(next); commitSelectedShapeUpdate({ fillColor: next }) }}
                              className={['flex h-8 w-8 items-center justify-center rounded border text-xs font-medium transition-colors', newFillColor === 'none' ? 'bg-background text-muted-foreground hover:border-primary' : 'bg-secondary text-foreground'].join(' ')}>
                              {newFillColor === 'none' ? '∅' : '✓'}
                            </button>
                          </div>
                        </div>
                      )}
                      <div className={['flex items-center justify-between gap-2', shapeType === 'line' || (selectedIsShape && (selectedAnn as ShapeAnnotation)?.shape === 'line') ? 'mb-2' : ''].join(' ')}>
                        <label className="text-xs text-muted-foreground">Width</label>
                        <input type="number" min={1} max={20} value={newStrokeWidth}
                          onChange={(e) => setNewStrokeWidth(Number(e.target.value))}
                          onBlur={(e) => commitSelectedShapeUpdate({ strokeWidth: Number(e.target.value) })}
                      ...
                      {(shapeType === 'line' || (selectedIsShape && (selectedAnn as ShapeAnnotation)?.shape === 'line')) && (
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex gap-1">
                            <button type="button" title="Arrow at start"
                              onClick={() => { const v = !arrowStart; setArrowStart(v); commitSelectedShapeUpdate({ arrowStart: v }) }}
                              className={['flex h-8 w-8 items-center justify-center rounded border text-sm font-bold transition-colors', arrowStart ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:border-primary'].join(' ')}>
                              {'<'}
                            </button>
                            <button type="button" title="Arrow at end"
                              onClick={() => { const v = !arrowEnd; setArrowEnd(v); commitSelectedShapeUpdate({ arrowEnd: v }) }}
                              className={['flex h-8 w-8 items-center justify-center rounded border text-sm font-bold transition-colors', arrowEnd ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:border-primary'].join(' ')}>
                              {'>'}
                            </button>
                          </div>
                        </div>
                  ...
                  {(selectedIsDraw || mode === 'draw') && !(selectedIsShape || mode === 'add-shape') && (
                    <>
                      <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  {!selectedIsShape && !selectedIsDraw && mode !== 'add-shape' && mode !== 'draw' && (
                    <>
                      <div className="mb-2 flex items-center justify-between gap-2">
                        ...
                        <input type="number" min={6} max={96} value={newFontSize}
                          onChange={(e) => { const v = Number(e.target.value); setNewFontSize(v); updateSelected({ fontSize: v }) }}
                          onBlur={(e) => commitSelectedUpdate({ fontSize: Number(e.target.value) })}
                          className="w-16 rounded border bg-background px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-primary" />
                      </div>
                      <div className="mb-2 flex items-center justify-between gap-2">
                        <label className="text-xs text-muted-foreground">Color</label>
                        <ColorPicker
                          value={newColor}
                          onChange={(hex) => { setNewColor(hex); updateSelected({ color: hex }) }}
                          onBlur={(hex) => commitSelectedUpdate({ color: hex })}
                        />
                      </div>
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <ColorPicker
                            ...
                            onChange={(hex) => { setNewBgColor(hex); updateSelected({ bgColor: hex }) }}
                            onBlur={(hex) => commitSelectedUpdate({ bgColor: hex })}
                          />
                          <button type="button"
                            title={newBgColor === 'none' ? 'Enable background' : 'Remove background'}
                            onClick={() => { const next = newBgColor === 'none' ? '#ffffff' : 'none'; setNewBgColor(next); updateSelected({ bgColor: next }) }}
                            className={['flex h-8 w-8 items-center justify-center rounded border text-xs font-medium transition-colors', newBgColor === 'none' ? 'bg-background text-muted-foreground hover:border-primary' : 'bg-secondary text-foreground'].join(' ')}>
                            {newBgColor === 'none' ? '∅' : '✓'}
                          </button>
                        </div>
                      </div>
                ...
                <div className="flex-1 overflow-y-auto p-3">
                  <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    Annotations{annotations.length > 0 && <span className="ml-1 font-normal normal-case">({annotations.length})</span>}
                  </p>
                  {annotations.length === 0 ? (
                    ...
                    <ul className="space-y-1">
                      {annotations.map((ann) => (
                        <li key={ann.id}>
                          <button type="button"
                            onClick={() => { setSelectedId(ann.id); setCurrentPage(ann.page) }}
                            className={['flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-xs transition-colors hover:bg-muted', ann.id === selectedId ? 'bg-muted font-medium' : ''].join(' ')}>
                            {isSignature(ann) ? (
                              <PenLine className="h-3 w-3 shrink-0 text-muted-foreground" />
                            ) : isImage(ann) ? (
                              <img src={ann.dataUrl} alt="" className="h-4 w-4 shrink-0 rounded object-contain" />
                            ) : isShape(ann) ? (
                              ann.shape === 'line'
                                ? <Minus className="h-3 w-3 shrink-0" style={{ color: ann.strokeColor }} />
                                : ann.shape === 'circle'
                                ? <Circle className="h-3 w-3 shrink-0" style={{ color: ann.strokeColor }} />
                                : <Square className="h-3 w-3 shrink-0" style={{ color: ann.strokeColor }} />
                            ) : isDraw(ann) ? (
                              <PencilLine className="h-3 w-3 shrink-0" style={{ color: ann.strokeColor }} />
                            ) : (
                              <span className="h-3 w-3 shrink-0 rounded-sm border" style={{ backgroundColor: ann.color }} />
                            )}
                            <span className="min-w-0 flex-1 truncate">{isSignature(ann) ? 'Signature' : isImage(ann) ? ann.label : isShape(ann) ? (ann.shape === 'line' ? 'Line' : ann.shape) : isDraw(ann) ? 'Drawing' : ann.text}</span>
                            <span className="shrink-0 text-muted-foreground">p{ann.page}</span>
                          </button>
                        </li>
                      ))}
                    </ul>
                  ...
                  {selectedId && (
                    <Button variant="outline" size="sm" className="mt-3 w-full gap-1.5 text-destructive hover:text-destructive" onClick={deleteSelected}>
          ...
          <div className="flex gap-3">
            ...
            <div className="lg:hidden">
              <Sheet>
                <SheetTrigger asChild>
                  <Button variant="outline" size="icon" className="h-9 w-9" aria-label="Open tools">
                ...
                <SheetContent side="left" className="w-56 p-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <Button variant={mode === 'select' ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start gap-2"
                    ...
                    <Button variant={mode === 'area-ocr' ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start gap-2"
                      onClick={() => setMode('area-ocr')}>
                      <CaseSensitive className="h-4 w-4" /> OCR Area
                    </Button>
                    <Button variant={pageOcrStatus === 'running' ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start gap-2"
                      disabled={pageOcrStatus === 'running'} onClick={handlePageOcr}>
                      <FileSearch className={['h-4 w-4', pageOcrStatus === 'running' ? 'animate-pulse' : ''].join(' ')} /> Read Page
                    </Button>
                    ...
                    <Button variant={mode === 'draw' ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start gap-2"
                      onClick={() => setMode(mode === 'draw' ? 'select' : 'draw')}>
                      <Pencil className="h-4 w-4" /> Draw
                    </Button>
                    <Button variant={mode === 'add-signature' ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start gap-2"
                      onClick={() => { setMode('add-signature'); setShowSigModal(true) }}>
            ...
            <div className="min-w-0 flex-1 space-y-2">
              {/* Mode hints */}
              {mode === 'add-text' && (
                <p className="text-center text-xs text-muted-foreground">Click anywhere on the page to place a text element</p>
              )}
              {mode === 'area-scan' && (
                <p className={[
                  'text-center text-xs',
                  scanStatus === 'found' || scanStatus === 'ocr-found' ? 'text-green-600 dark:text-green-400' :
                  scanStatus === 'notfound' ? 'text-amber-600 dark:text-amber-400' :
                  scanStatus === 'scanning' ? 'text-muted-foreground animate-pulse' :
                  'text-muted-foreground',
                ].join(' ')}>
                  {scanStatus === 'found' ? 'QR/barcode detected — annotation added' :
                   scanStatus === 'ocr-found' ? 'Text extracted via OCR — annotation added' :
                   scanStatus === 'notfound' ? 'Nothing found in selection' :
              ...
              {mode === 'area-ocr' && (
                <p className={[
                  'text-center text-xs',
                  scanStatus === 'ocr-found' ? 'text-green-600 dark:text-green-400' :
                  scanStatus === 'notfound' ? 'text-amber-600 dark:text-amber-400' :
                  scanStatus === 'scanning' ? 'text-muted-foreground animate-pulse' :
                  'text-muted-foreground',
                ].join(' ')}>
                  {scanStatus === 'ocr-found' ? 'Text extracted — annotation added' :
                   scanStatus === 'notfound' ? 'No text found in selection' :
                   scanStatus === 'scanning' ? 'Recognising text…' :
                   'Drag to select an area to extract text with OCR'}
                </p>
              )}
              {mode === 'add-shape' && (
                <p className="text-center text-xs text-muted-foreground">
                  Drag to draw a {shapeType === 'circle' ? 'circle / ellipse' : shapeType === 'line' ? 'line / arrow' : 'rectangle'} — configure style in the panel
                </p>
              )}
              {mode === 'draw' && (
                <p className="text-center text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      {pageOcrStatus === 'done' && pageOcrText && (
                        <Button size="icon" variant="ghost" className="h-6 w-6" onClick={handleOcrCopy} title="Copy to clipboard">
                          {ocrCopied ? <Check className="h-3.5 w-3.5 text-green-600" /> : <Copy className="h-3.5 w-3.5" />}
                        </Button>
                      )}
                      <Button size="icon" variant="ghost" className="h-6 w-6 text-muted-foreground" onClick={() => { setPageOcrStatus('idle'); setPageOcrText('') }} title="Dismiss">
                        <X className="h-3.5 w-3.5" />
                      </Button>
                    </div>
                  </div>
                  {pageOcrStatus === 'running' && (
                    <p className="animate-pulse text-xs text-muted-foreground">Recognising text on page {currentPage}…</p>
                  )}
                  {pageOcrStatus === 'error' && (
                    <p className="text-xs text-amber-600 dark:text-amber-400">No text found on this page.</p>
                  )}
                  {pageOcrStatus === 'done' && pageOcrText && (
                    <textarea
                      readOnly
              ...
              {showGenPanel && (
                <div className="rounded-lg border bg-card p-3 space-y-2">
                  ...
                  <input
                    type="text"
                    placeholder={mode === 'add-qr' ? 'https://example.com' : 'Enter barcode value'}
                    value={genText}
                    onChange={(e) => setGenText(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') { mode === 'add-qr' ? placeQrCode() : placeBarcodeImage() } }}
                  ...
                  <Button
                    ...
                    onClick={mode === 'add-qr' ? placeQrCode : placeBarcodeImage}
                    disabled={!genText.trim()}
                  >
                    {mode === 'add-qr' ? <QrCode className="h-3.5 w-3.5" /> : <Barcode className="h-3.5 w-3.5" />}
                    Insert into PDF
                  </Button>
                </div>
              )}
              ...
              <div className="overflow-auto rounded-lg border bg-muted/30">
                <div className="relative mx-auto" style={{ width: 'fit-content' }}>
                  <canvas ref={canvasRef} aria-label={`PDF page ${currentPage} of ${totalPages}`} className="block" />

                  {/* Overlay */}
                  <div
                    ref={overlayRef}
                    className={['absolute inset-0 select-none', overlayCursor].join(' ')}
                    style={(mode === 'area-scan' || mode === 'area-ocr' || mode === 'add-shape' || mode === 'draw') ? { touchAction: 'none' } : undefined}
                    ...
                    {shapeRect && mode === 'add-shape' && shapeType !== 'line' && (
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: newFillColor !== 'none' ? `${newFillColor}33` : undefined,
                        }}
                      />
                    )}
                    {shapeRect && mode === 'add-shape' && shapeType === 'line' && (
                      <svg className="pointer-events-none absolute inset-0 overflow-visible" style={{ width: '100%', height: '100%' }}>
                        <defs>
                          <marker id="preview-arrow-end" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
                            <path d="M0,0 L8,4 L0,8 Z" fill={newStrokeColor} />
                          </marker>
                          <marker id="preview-arrow-start" markerWidth="8" markerHeight="8" refX="2" refY="4" orient="auto">
                            <path d="M8,0 L0,4 L8,8 Z" fill={newStrokeColor} />
                          </marker>
                        </defs>
                        <line
                          x1={shapeRect.x1} y1={shapeRect.y1} x2={shapeRect.x2} y2={shapeRect.y2}
                          stroke={newStrokeColor} strokeWidth={newStrokeWidth} strokeLinecap="round"
                          markerEnd={arrowEnd ? 'url(#preview-arrow-end)' : undefined}
                          markerStart={arrowStart ? 'url(#preview-arrow-start)' : undefined}
                        />
                      </svg>
                    )}
                    ...
                    {mode === 'draw' && livePoints.length >= 2 && (() => {
                      ...
                      return (
                        <svg className="pointer-events-none absolute inset-0" width={cw} height={ch} style={{ overflow: 'visible' }}>
                          <polyline
                            points={livePoints.map((p) => `${p.x * cw},${p.y * ch}`).join(' ')}
                            fill="none"
                    ...
                    {pageAnnotations.map((ann) => {
                      ...
                      if (isShape(ann)) {
                        if (ann.shape === 'line') {
                          ...
                          return (
                            <svg key={ann.id} className="absolute inset-0 overflow-visible" style={{ width: w, height: h }}>
                              <defs>
                                <marker id={mid} markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
                                  <path d="M0,0 L8,4 L0,8 Z" fill={ann.strokeColor} />
                                </marker>
                                <marker id={`${mid}-rev`} markerWidth="8" markerHeight="8" refX="2" refY="4" orient="auto">
                                  <path d="M8,0 L0,4 L8,8 Z" fill={ann.strokeColor} />
                                </marker>
                              </defs>
                              {/* Wide transparent hit area */}
                              <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="transparent" strokeWidth={Math.max(ann.strokeWidth, 14)} style={{ pointerEvents: 'stroke', cursor: 'move' }}
                                onPointerDown={(e) => { e.stopPropagation(); handleAnnotationPointerDown(e as React.PointerEvent, ann.id) }}
                                onPointerMove={(e) => { handleAnnotationPointerMove(e as React.PointerEvent); handleResizePointerMove(e as React.PointerEvent) }}
                                onPointerUp={() => { handleAnnotationPointerUp(); handleResizePointerUp() }}
                              />
                              <line
                                x1={x1} y1={y1} x2={x2} y2={y2}
                                stroke={ann.strokeColor} strokeWidth={ann.strokeWidth} strokeLinecap="round"
                                markerEnd={ann.arrowEnd ? `url(#${mid})` : undefined}
                                markerStart={ann.arrowStart ? `url(#${mid}-rev)` : undefined}
                                style={{ pointerEvents: 'none' }}
                              />
                              {isSelected && (
                                <>
                                  <circle cx={x1} cy={y1} r={6} fill="white" stroke="hsl(var(--primary))" strokeWidth={2}
                                    style={{ pointerEvents: 'all', cursor: 'crosshair' }}
                                    onPointerDown={(e) => handleLineEndpointDown(e, ann.id, 'start')} />
                                  <circle cx={x2} cy={y2} r={6} fill="white" stroke="hsl(var(--primary))" strokeWidth={2}
                                    style={{ pointerEvents: 'all', cursor: 'crosshair' }}
                                    onPointerDown={(e) => handleLineEndpointDown(e, ann.id, 'end')} />
                                </>
                              )}
                            </svg>
                          )
                        ...
                        return (
                          <div
                            key={ann.id}
                            style={{
                              position: 'absolute', left, top, width: shapeW, height: shapeH,
                              border: `${ann.strokeWidth}px solid ${ann.strokeColor}`,
                              backgroundColor: ann.fillColor === 'none' ? undefined : ann.fillColor,
                              borderRadius: ann.shape === 'circle' ? '50%' : undefined,
                              cursor: 'move', boxSizing: 'border-box',
                            }}
                            className={isSelected ? 'outline outline-2 outline-offset-2 outline-primary' : ''}
                            onPointerDown={(e) => { e.stopPropagation(); handleAnnotationPointerDown(e, ann.id) }}
                            onPointerMove={(e) => { handleAnnotationPointerMove(e); handleResizePointerMove(e) }}
                            onPointerUp={() => { handleAnnotationPointerUp(); handleResizePointerUp() }}
                          >
                            {isSelected && (() => {
                              const bg = ann.fillColor !== 'none' ? ann.fillColor : ann.strokeColor
                              return (
                                <div
                                  style={{ position: 'absolute', right: -5, bottom: -5, width: 14, height: 14, cursor: 'se-resize', backgroundColor: bg, borderColor: contrastColor(bg) }}
                      ...
                      if (isDraw(ann)) {
                        ...
                        const pts = ann.points.map((p) => `${p.x * w - minX},${p.y * h - minY}`).join(' ')
                        return (
                          <svg
                            key={ann.id}
                            style={{ position: 'absolute', left: minX, top: minY, width: svgW, height: svgH, overflow: 'visible', cursor: 'move' }}
                            className={isSelected ? 'outline outline-2 outline-offset-2 outline-primary rounded' : ''}
                            onPointerDown={(e) => { e.stopPropagation(); handleAnnotationPointerDown(e, ann.id) }}
                            onPointerMove={handleAnnotationPointerMove}
                            onPointerUp={handleAnnotationPointerUp}
                          >
                            <polyline points={pts} fill="none" stroke={ann.strokeColor} strokeWidth={ann.strokeWidth} strokeLinejoin="round" strokeLinecap="round" />
                            {/* Wide transparent hit area so thin lines stay clickable */}
                            <polyline points={pts} fill="none" stroke="transparent" strokeWidth={Math.max(ann.strokeWidth, 14)} />
                          </svg>
                        )
                      }
                      ...
                      if (isSignature(ann)) {
                        ...
                        return (
                          <div
                            key={ann.id}
                            style={{ position: 'absolute', left, top, width: sigW, height: sigH, cursor: 'move' }}
                            className={isSelected ? 'outline outline-2 outline-offset-2 outline-primary' : ''}
                            onPointerDown={(e) => { e.stopPropagation(); handleAnnotationPointerDown(e, ann.id) }}
                            onPointerMove={(e) => { handleAnnotationPointerMove(e); handleResizePointerMove(e) }}
                            onPointerUp={() => { handleAnnotationPointerUp(); handleResizePointerUp() }}
                          >
                            ...
                            {isSelected && (
                              <div
                                style={{ position: 'absolute', right: -5, bottom: -5, width: 14, height: 14, cursor: 'se-resize', backgroundColor: '#ffffff', borderColor: '#222222' }}
                                className="rounded-sm border-2"
                                onPointerDown={(e) => handleResizePointerDown(e, ann.id)}
                                onPointerMove={handleResizePointerMove}
                                onPointerUp={handleResizePointerUp}
                      ...
                      if (isImage(ann)) {
                        ...
                        return (
                          <div
                            key={ann.id}
                            style={{ position: 'absolute', left, top, width: imgW, height: imgH, cursor: 'move' }}
                            className={isSelected ? 'outline outline-2 outline-offset-2 outline-primary' : ''}
                            onPointerDown={(e) => { e.stopPropagation(); handleAnnotationPointerDown(e, ann.id) }}
                            onPointerMove={(e) => { handleAnnotationPointerMove(e); handleResizePointerMove(e) }}
                            onPointerUp={() => { handleAnnotationPointerUp(); handleResizePointerUp() }}
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            color: ann.color, lineHeight: 1,
                            backgroundColor: ann.bgColor === 'none' ? undefined : ann.bgColor,
                          }}
                          className={[
                        ...
                        >
                          {isEditing ? (
                            <input
                              ...
                              onClick={(e) => e.stopPropagation()}
                              style={{ fontSize: 'inherit', color: 'inherit', background: 'transparent', border: 'none', outline: 'none', width: Math.max(60, ann.text.length * 0.65 * ann.fontSize * scale) }}
                            />
                          ) : (
                            <span>{ann.text}</span>
                          )}
                    ...
                    <>
                      <div className="mb-2 flex items-center justify-between gap-2">
                        ...
                        <ColorPicker
                          ...
                          onBlur={(hex) => commitSelectedShapeUpdate({ strokeColor: hex })}
                        />
                      </div>
                      {shapeType !== 'line' && !(selectedIsShape && (selectedAnn as ShapeAnnotation)?.shape === 'line') && (
                        <div className="mb-2 flex items-center justify-between gap-2">
                        ...
                        <li key={ann.id}>
                          <button type="button"
                            ...
                            <span className="min-w-0 flex-1 truncate">
                              {isSignature(ann) ? 'Signature' : isImage(ann) ? ann.label : isShape(ann) ? (ann.shape === 'line' ? 'Line' : ann.shape) : isDraw(ann) ? 'Drawing' : ann.text}
                            </span>
                            <span className="shrink-0 text-muted-foreground">p{ann.page}</span>
                          </button>
                        </li>
      ...
      {showSigModal && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) { setShowSigModal(false); setMode('select') } }}
        >
          <div className="w-full max-w-md rounded-xl border bg-card shadow-xl">
            <div className="flex items-center justify-between border-b p-4">
              <h2 className="text-sm font-semibold">Signature</h2>
              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setShowSigModal(false); setMode('select') }}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            ...
            <div className="flex border-b">
              <button
                ...
                className={['flex-1 py-2 text-sm font-medium transition-colors', !sigDrawing ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground hover:text-foreground'].join(' ')}
              >
                Saved
              </button>
              <button
                ...
                <>
                  {savedSignatures.length === 0 ? (
                    <div className="flex flex-col items-center gap-3 py-8 text-center">
                      ...
                      <Button size="sm" onClick={() => setSigDrawing(true)}>Draw your first signature</Button>
                    </div>
                  ) : (
                    <ul className="space-y-2">
                      {savedSignatures.map((sig) => (
                        <li key={sig.id} className="flex items-center gap-2 rounded-lg border bg-muted/30 p-2">
                          <img src={sig.dataUrl} alt={sig.name} className="h-10 w-24 shrink-0 rounded border bg-white object-contain" />
                          {renamingId === sig.id ? (
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') { e.preventDefault(); commitRename(sig.id) }
                            ...
                            <span className="min-w-0 flex-1 truncate text-sm">{sig.name}</span>
                          )}
                          <Button
                            size="icon" variant="ghost"
                            className="h-7 w-7 shrink-0 text-muted-foreground hover:text-foreground"
                            title="Rename"
                            onClick={() => { setRenamingId(sig.id); setRenameValue(sig.name) }}
                          >
                            <Pencil className="h-3.5 w-3.5" />
                          </Button>
                          <Button size="sm" onClick={() => placeSignature(sig)}>Use</Button>
                          <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0 text-muted-foreground hover:text-destructive" onClick={() => deleteSignature(sig.id)}>
                            <Trash2 className="h-3.5 w-3.5" />
                          </Button>
                        </li>
                      ))}
              ...
              {sigDrawing && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="gap-1.5" onClick={clearSigCanvas}>
                      <RotateCcw className="h-3.5 w-3.5" /> Clear
                    </Button>
                    <Button size="sm" className="flex-1" onClick={saveSignature}>
                      Save signature
                    </Button>
                  </div>
                </div>
